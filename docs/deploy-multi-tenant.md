...
| `retaguarda` | por tenant | camada detectiva (7-F): parcelas/pagamentos → contratos do tenant (`membershipDoTenant`). Grava/reconcilia achados em `retaguarda_achado` (por tenant). Não muta dado de negócio (LGPD art. 20). |
| `alertar-sla` | por tenant | exceções abertas → `titular.tenant_id` (via `carregarPainelSLA`, escopado por `membershipDoTenant`). Digest diário: um e-mail interno com os SLAs já estourados; reenvia no dia seguinte se seguirem estourados. |
