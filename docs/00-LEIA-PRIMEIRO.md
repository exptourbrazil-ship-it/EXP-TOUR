...
**Padrões obrigatórios em todo código novo:** toda mutação por função nomeada única (valida papel → transação → grava evento → audit → enfileira notificação); nenhuma credencial no cliente; parâmetros de negócio (spread 5%, IOF 3,5%, buffers, SLAs) em config por instância, nunca hardcoded; `valor_original` intocável; não reintroduzir o banner de simular câmbio; RLS como defesa em profundidade.
