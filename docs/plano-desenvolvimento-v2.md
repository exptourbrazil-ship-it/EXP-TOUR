...
8. **Abas Embarque, Viagem e Retorno** (estados 6–8): checklist de embarque versão 1 (estática por destino antes do motor de regras), contatos/endereços na Viagem, e o Retorno com certificado no cofre + NPS + convite Google + indicação, que é o de melhor retorno por esforço.
   ...
   - **Embarque — entregue** (aba `/embarque` ativa): checklist de pré-embarque híbrido — itens de documento marcam sozinhos pelo cofre + tarefas manuais salvas em `embarque_checklist` (rota `POST /api/embarque/checklist`); listas por destino via `contratos.pais_destino` (`canada`/`eua`/`nova_zelandia` + base; Canadá exige visto de estudo + B1/B2 de trânsito). Config em `src/lib/embarque.ts`.
   - **Viagem — entregue** (aba `/viagem` ativa): contatos da EXP Tour (WhatsApp/site), número de emergência por destino (`src/lib/viagem.ts`), escola/acomodação/contato local como dados estruturados em `viagem_info` (com "Ver no mapa") e atalhos para os documentos úteis do cofre.
   - **Item 8 concluído** (abas Embarque, Viagem e Retorno no ar). Formulário admin para preencher `viagem_info` entregue (`/admin/viagem` + `POST /api/admin/viagem-info`, com auditoria). Pendências futuras deste bloco: consulado no bloco de emergência e o checklist versão "motor de regras" (item 9).
