...
create table if not exists tenant (
  ...
  chat_url text, -- link do chat (ex.: Chat da Forio) exibido como botao na aba "Sobre nos"
...
-- ── Anexo III / Contrato v3.1 — entidades NOVAS (aplicado via
-- migracao-anexo3-v3.1.sql). Complementam politica_retencao (escada por campus,
-- já existente). Ver src/lib/anexo3-entidades.ts (domínio puro/validadores) e
-- docs/forio-especificacao-anexo3-sistema.md (§3).

-- Componente Educacional (Cláusula 1.1.g.1): base da Remuneração por Serviços
-- Prestados, ≠ Custo do Programa. Default educacional; terceiros marcam 'terceiro'.
alter table if exists product
  add column if not exists componente text not null default 'educacional'
  check (componente in ('educacional','terceiro'));

-- Taxas que compõem a Entrada, por campus (além de `fee`).
create table if not exists taxa_obrigatoria (
  ...
  duracao_minima_semanas int,
  reembolsavel boolean not null default false,
  vencimento_dias int not null default 0 check (vencimento_dias >= 0),
...
-- Exigência de antecipação: REGRA por campus que ativa a Cláusula 7.5 (distinta da
...
-- Política do campus (1:1): intake máximo vendável, reembolso, proteção estudantil,
-- fonte da política, país do calendário para regras em dias úteis. Moeda = campus.base_currency.
create table if not exists campus_politica (
  campus_id uuid primary key references campus(id) on delete cascade,
  tenant_id uuid not null references tenant(id),
  intake_maximo_vendavel date,
...
create table if not exists promotion (
  ...
  -- So para free_units: faixa de preco a usar no lugar da quantidade contratada.
  free_units_tier_quantity int,
...
-- F3.4 — DISPONIBILIDADE lida por IA -> proposta PENDENTE (uma linha por leitura de
-- material; `plano` = itens criar/alterar/igual ja comparados ao publicado). So a
-- aprovacao grava product_availability/accommodation_availability (salvarIntake/
-- salvarPeriodo). Migracao: supabase/migracao-availability-submission.sql (prod 2026-09-16).
...
-- Fase B1 — staging de CONTEUDO DE PROGRAMA proposto pelo fornecedor
-- ("propoe -> admin aprova"). Aprovacao materializa em product_content /
-- product_media / program_detail. Ver migracao-content-submission.sql.
...
-- Fase B2 — conteudo de ESCOLA (campus): colunas novas + staging.
-- Ver migracao-campus-conteudo.sql.
...
create table if not exists material (
  ...
  tenant_id uuid not null references tenant(id),
  supplier_id uuid not null references supplier(id) on delete cascade,
  tipo text not null check (tipo in ('brochura','price_list','promocao','calendario','foto','video','apresentacao','midia_kit','logotipo','termos','outro')),
  titulo text not null,
...
-- ============================================================================
-- Ajuste sazonal de acomodacao (migracao-ajuste-sazonal.sql, prod 19/09/2026)
-- ============================================================================
-- Alta/baixa temporada por semana, uma linha por intervalo; sem ano = recorrente.
-- POSITIVO = suplemento, NEGATIVO = desconto; entra como LINHA SEPARADA na cotacao.
create table if not exists seasonal_adjustment (
  ...
  -- ate 7 semanas, 26 de 8 a 23 e 11 acima disso.
