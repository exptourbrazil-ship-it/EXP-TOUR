...
// Ações de aprovação/rejeição do conteúdo de escola (Fase B2).
export default function ConteudoEscolaAprovacaoClient({ id }: { id: string }) {
  ...
  async function agir(acao: "aprovar" | "rejeitar") {
    ...
    setErro(null);
    try {
      const r = await fetch("/api/admin/campus-content-submissions", {
        method: "POST",
      ...
      else router.push("/admin/conteudo-escolas");
    } catch {
