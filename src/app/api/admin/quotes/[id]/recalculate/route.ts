...
import { recalculateQuote } from "@/lib/quote-service";
import { getSupabase, guardCatalog, bad, fail, okData } from "@/lib/catalog-route";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

// POST /api/admin/quotes/[id]/recalculate — recalcula a cotacao (so em rascunho).
export async function POST(
  ...
  try {
    ...
    const totals = await recalculateQuote(
      supabase,
      { tenantId, quoteId },
      { usuario: g.usuario, ip: g.ip },
    );
    return okData(totals);
  } catch (err) {
    return fail(err);
  }
}
