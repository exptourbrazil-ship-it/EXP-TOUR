...
//
// Protegida pelo mesmo segredo simples do cambio manual
// (ADMIN_CAMBIO_SECRET), enviado como Bearer token, ja que a
...
export async function POST(request: Request) {
  ...
  const { data, error } = await supabase
    ...
    .eq("id", titularId)
    .select("id, nome_completo, data_inicio")
    .maybeSingle()

  if (error || !data) {
    return NextResponse.json({ ok: false, erro: "Nao foi possivel salvar a data de inicio." }, { status: 500 })
  }

  return NextResponse.json({ ok: true, titular: data })
}
